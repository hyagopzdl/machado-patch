      (() => {
        var { useState: b, useEffect: He, useCallback: H, useMemo: X } = React;
        const {
        ...
        function to() {
          ...
          He(() => {
            ...
            let handler = (snapshot) => {
              ...
              presenceRef.set({ online: true, updatedAt: firebase.database.ServerValue.TIMESTAMP });
            };
          ...
          function acceptTradeOffer(offerId) {
            ...
            // Fast UX validation. The same rules are checked again atomically in Firebase.
      ...
      `,
        ...
        function ChampionshipRulesModal({ tournament, onClose }) {
          ...
          return React.createElement(ee,{title:"Regras da competição",onClose},
            ...
            !isCup&&section("💸","Mercado",React.createElement(React.Fragment,null,row("Mercado",market.isOpen===false?"Fechado":"Aberto",market.isOpen!==false),row("Limite de overall atual",market.freePlayerOverallLimit&&market.freePlayerOverallLimit.enabled?String(Number(market.freePlayerOverallLimit.maxOverall||99)):"Sem limite"),React.createElement("div",{style:{fontSize:10.5,fontWeight:850,textTransform:"uppercase",letterSpacing:".08em",color:"var(--muted)",marginTop:14}},"Taxas do Lobo Pidão"),row("Depreciação",`${market.depreciationPct!=null?market.depreciationPct:10}%`,false,true),row("Elenco-base",`${market.initialRosterDepreciationPct!=null?market.initialRosterDepreciationPct:50}%`,false,true))),
