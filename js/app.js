      (() => {
        ...
        function to() {
          ...
          function updateEconomyRules(winReward, scoringDrawReward, scorelessDrawReward, lossReward, goalReward, redCardPenalty, championPrize, topScorerPrize) {
            if (!R) return;
            let economySettings = { ...((R&&R.economySettings)||{}), version: 2, winReward: Math.round(Math.max(0, Number(winReward)||0)), scoringDrawReward: Math.round(Math.max(0, Number(scoringDrawReward)||0)), scorelessDrawReward: Math.round(Math.max(0, Number(scorelessDrawReward)||0)), lossReward: Math.round(Math.max(0, Number(lossReward)||0)), goalReward: Math.round(Math.max(0, Number(goalReward)||0)), redCardPenalty: Math.round(Math.max(0, Number(redCardPenalty)||0)) };
          ...
          function updateBalanceLoanSettings(values) {
            if(!R)return;
            let current={...((R.economySettings)||{})}, normalized={...balanceLoanSettingsOf(R),...values};
            Object.keys(normalized).forEach(key=>{if(key!=="enabled"&&key!=="allowTopUps"&&key!=="referenceMethod")normalized[key]=Math.max(0,Math.round(Number(normalized[key])||0));});
            normalized.enabled=values.enabled===true;normalized.allowTopUps=values.allowTopUps!==false;
            ae(m.map(item=>item.id===R.id?{...item,economySettings:{...current,balanceLoanSettings:normalized}}:item));
          }
          function grantBalanceLoan(teamId, requestedAmount) {
            if(!R||!teamId)return;
            let analysis=balanceLoanAnalysis(R,p,teamId), amount=Math.max(0,Math.round(Number(requestedAmount)||0));
            if(!amount)return;
            let settings=analysis.settings, existing=analysis.loan, totalGranted=Math.round(Number(existing&&existing.totalGranted)||0);
            if(settings.maximumTotalAmount>0&&totalGranted+amount>settings.maximumTotalAmount){window.alert(`O teto acumulado deste usuário é ${L(settings.maximumTotalAmount)}.`);return;}
            let team=p.find(item=>String(item.id)===String(teamId));if(!team)return;
            let now=Date.now(), before=Number(team.budget)||0, grant={id:_(),amount,createdAt:now,createdByProfileId:te&&te.id||null,snapshot:{referenceGames:analysis.referenceGames,played:analysis.played,rawGap:analysis.rawGap,compensableGames:analysis.compensableGames,averageReward:analysis.averageReward,compensationPercentage:settings.compensationPercentage,calculated:analysis.calculated,suggested:existing?analysis.topUpSuggested:analysis.suggested}};
            let loan={teamId,totalGranted:totalGranted+amount,totalPaid:Math.round(Number(existing&&existing.totalPaid)||0),remaining:Math.round(Number(existing&&existing.remaining)||0)+amount,status:"active",repaymentPercentage:existing?Math.round(Number(existing.repaymentPercentage)||settings.repaymentPercentage):settings.repaymentPercentage,createdAt:existing&&existing.createdAt||now,updatedAt:now,grants:[...(existing&&Array.isArray(existing.grants)?existing.grants:[]),grant]};
            let loans={...balanceLoansOf(R),[String(teamId)]:loan}, context=R.context||{}, teams=p.map(item=>String(item.id)===String(teamId)?{...item,budget:before+amount}:item), transactions=Array.isArray(context.financialTransactions)?[...context.financialTransactions]:[];
            transactions.unshift(financeEntry(existing?"balance_loan_topup":"balance_loan_grant",teamId,amount,existing?"Complemento do empréstimo de equilíbrio":"Empréstimo de equilíbrio",grant.id,before,now));
            ae(m.map(item=>item.id===R.id?{...item,economySettings:{...((item.economySettings)||{}),balanceLoans:loans},context:{...(item.context||{}),teams,financialTransactions:transactions}}:item));
          }
          ...
          function saveQuickMatch(data) {
            ...
            let match={...baseMatch,economyRewards,economySettlement:{version:3,settledAt:now,retroactive:false,settingsSnapshot:{...rewards},loanRepayments,breakdown:{[left.id]:leftBreakdown,[right.id]:rightBreakdown}}};
            let context = R.context || {}, teams = p.map((team)=>{let gross=Math.round(Number(economyRewards[team.id])||0),repayment=Math.round(Number(loanRepayments[team.id])||0),net=gross-repayment;return net?{...team,budget:(Number(team.budget)||0)+net}:team;});
      ...
      `,
                    ...
                    balanceHistoryOpen && R && (ProfileTeam || isAdminProfile(te)) && React.createElement(BalanceHistoryModal, { tournament:R, tournamentId:R.id, team: ProfileTeam || null, teams:p, canRollback:isAdminProfile(te), showAll:isAdminProfile(te)&&!ProfileTeam, onRollback:rollbackMarketPurchase, onClose:()=>setBalanceHistoryOpen(false) }),
        ...
        function BalanceHistoryModal({ tournament, tournamentId, team, teams = [], canRollback, showAll = false, onRollback, onClose }) {
          let [items,setItems]=b([]),[loading,setLoading]=b(true),[loadingMore,setLoadingMore]=b(false),[hasMore,setHasMore]=b(false),[cursor,setCursor]=b(null),[error,setError]=b(null),[loanInfoOpen,setLoanInfoOpen]=b(false);
          let loan=team?balanceLoansOf(tournament)[String(team.id)]:null;
          let loanProgress=loan&&loan.totalGranted?Math.min(100,Math.round((Number(loan.totalPaid)||0)/(Number(loan.totalGranted)||1)*100)):0;
          ...
          let loanCard=loan&&!showAll&&React.createElement("div",{style:{padding:16,borderRadius:18,background:"var(--surface-soft)",border:"1px solid var(--border)",marginBottom:16}},
            React.createElement("div",{style:{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center"}},
              React.createElement("div",null,React.createElement("div",{style:{fontSize:12,color:"var(--muted)"}},loan.status==="paid"?"Empréstimo concluído":"Empréstimo ativo"),React.createElement("strong",{style:{fontSize:18}},`${L(loan.remaining)} restantes`)),
              React.createElement("button",{onClick:()=>setLoanInfoOpen(true),title:"Como esse valor foi definido?",style:{width:32,height:32,borderRadius:999,border:"1px solid var(--border)",background:"var(--surface)",color:"var(--heading)",fontWeight:900}},"i")
            ),
            React.createElement("div",{style:{height:7,borderRadius:999,background:"var(--border)",overflow:"hidden",margin:"14px 0 10px"}},React.createElement("div",{style:{height:"100%",width:`${loanProgress}%`,background:"var(--green)"}})),
            React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,fontSize:11.5}},[["Recebido",loan.totalGranted],["Pago",loan.totalPaid],["Desconto",`${loan.repaymentPercentage}%`]].map(([label,value])=>React.createElement("div",{key:label},React.createElement("div",{style:{color:"var(--muted)",marginBottom:3}},label),React.createElement("strong",null,typeof value==="number"?L(value):value)))),
            React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:12,lineHeight:1.5}},"Uma parte das recompensas das próximas partidas é usada automaticamente para pagar o empréstimo.")
          );
          let infoModal=loanInfoOpen&&React.createElement("div",{onClick:()=>setLoanInfoOpen(false),style:{position:"fixed",inset:0,zIndex:1300,background:"rgba(0,0,0,.62)",display:"grid",placeItems:"center",padding:20}},React.createElement("div",{onClick:event=>event.stopPropagation(),style:{width:"min(430px,100%)",background:"var(--surface)",border:"1px solid var(--border)",borderRadius:22,padding:20}},
            React.createElement("div",{style:{fontSize:18,fontWeight:850,marginBottom:8}},"Como esse valor foi definido?"),
            React.createElement("div",{style:{fontSize:13,color:"var(--muted)",lineHeight:1.55}},"O valor considera sua diferença de partidas em relação aos participantes mais ativos e a média de recompensas recebidas por jogo."),
            loan&&loan.grants&&loan.grants.length&&React.createElement("div",{style:{marginTop:14,padding:14,borderRadius:14,background:"var(--surface-soft)",fontSize:12}},(()=>{let snap=loan.grants[loan.grants.length-1].snapshot||{};return React.createElement(React.Fragment,null,React.createElement("div",null,`Referência: ${snap.referenceGames||0} jogos · Você: ${snap.played||0}`),React.createElement("div",{style:{marginTop:5}},`Média: ${L(snap.averageReward||0)} por jogo · Compensação: ${snap.compensationPercentage||0}%`));})()),
            React.createElement("button",{onClick:()=>setLoanInfoOpen(false),style:{...M,...W,marginTop:16}},"Entendi")
          ));
          let modal=React.createElement("div",{className:"sports-modal-overlay",onClick:onClose,style:{position:"fixed",inset:0,zIndex:1200,display:"grid",placeItems:"center",padding:16,background:"rgba(0,0,0,.72)",backdropFilter:"blur(12px)"}},React.createElement("div",{onClick:(event)=>event.stopPropagation(),style:{width:"min(560px,100%)",maxHeight:"82vh",overflow:"auto",background:"var(--surface)",border:"1px solid var(--border)",borderRadius:24,padding:20}},
            React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},React.createElement("div",null,React.createElement("div",{style:{fontSize:12,color:"var(--muted)"}},showAll?"Mercado":"Saldo atual"),React.createElement("strong",{style:{fontSize:showAll?22:28,color:showAll?"var(--heading)":"var(--green)"}},showAll?"Histórico de transações":L(team&&team.budget||0))),React.createElement("button",{onClick:onClose,style:{width:34,height:34,borderRadius:"50%",border:"1px solid var(--border)",background:"var(--surface-soft)",color:"var(--heading)"}},"×")),
            loanCard,
            loading&&React.createElement("div",{style:{color:"var(--muted)",textAlign:"center",padding:28}},"Carregando histórico…"),
            error&&React.createElement("div",{style:{color:"var(--danger)",textAlign:"center",padding:18}},error),
            !loading&&items.map((item)=>React.createElement("div",{key:item.id,style:{display:"grid",gridTemplateColumns:"minmax(0,1fr) auto",gap:12,alignItems:"center",padding:"12px 0",borderBottom:"1px solid var(--border)"}},React.createElement("div",null,React.createElement("div",{style:{fontWeight:750,fontSize:13.5}},item.label||"Movimentação"),showAll&&React.createElement("div",{style:{fontSize:11,color:"var(--muted)",marginTop:3}},teamName(item.teamId)),React.createElement("div",{style:{fontSize:10.5,color:"var(--muted)",marginTop:3}},new Date(item.createdAt||Date.now()).toLocaleString("pt-BR"))),React.createElement("div",{style:{display:"flex",alignItems:"center",gap:9}},React.createElement("strong",{style:{color:Number(item.amount)>=0?"var(--green)":"var(--danger)"}},`${Number(item.amount)>=0?"+ ":"− "}${L(Math.abs(Number(item.amount)||0))}`),rollbackButton(item)))),
            !loading&&!items.length&&!error&&React.createElement("div",{style:{color:"var(--muted)",textAlign:"center",padding:24}},"Nenhuma movimentação registrada ainda."),
            hasMore&&React.createElement("button",{disabled:loadingMore,onClick:()=>fetchTransactions(false),style:{width:"100%",marginTop:16,padding:12,borderRadius:12,border:"1px solid var(--border)",background:"var(--surface-soft)",color:"var(--heading)",fontWeight:800,cursor:loadingMore?"wait":"pointer"}},loadingMore?"Carregando…":"Carregar mais 50"),
            infoModal
          ));
        ...
        function BalanceLoanAdmin({tournament,teams,onSave,onGrant}){
          let initial=balanceLoanSettingsOf(tournament),[values,setValues]=b(initial),[amounts,setAmounts]=b({});He(()=>setValues(balanceLoanSettingsOf(tournament)),[tournament&&tournament.id,tournament&&tournament.economySettings]);
          let setNumber=key=>event=>setValues(current=>({...current,[key]:Math.max(0,Math.round(Number(event.target.value)||0))}));
          let analyses=(teams||[]).filter(team=>team&&team.active!==false).map(team=>({team,analysis:balanceLoanAnalysis(tournament,teams,team.id)}));
          let field=(label,key,suffix="",helper="")=>React.createElement("label",{style:{fontSize:11,color:"var(--muted)"}},label,React.createElement("div",{style:{position:"relative",marginTop:6}},React.createElement("input",{type:"number",min:0,value:values[key],onChange:setNumber(key),style:{...q,paddingRight:suffix?38:12}}),suffix&&React.createElement("span",{style:{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",fontSize:11}},suffix)),helper&&React.createElement("div",{style:{fontSize:10.5,lineHeight:1.4,marginTop:5}},helper));
          return React.createElement("div",{style:{marginTop:16,paddingTop:16,borderTop:"1px solid var(--border)"}},
            React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,marginBottom:12}},React.createElement("div",null,React.createElement("div",{style:{fontSize:16,fontWeight:850}},"Empréstimos de equilíbrio"),React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:3}},"Compense parte das oportunidades de recompensa perdidas.")),React.createElement("label",{style:{display:"flex",alignItems:"center",gap:8,fontSize:12,fontWeight:800}},React.createElement("input",{type:"checkbox",checked:values.enabled,onChange:event=>setValues({...values,enabled:event.target.checked})}),"Ativo")),
            React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(175px,1fr))",gap:10}},
              field("Tolerância de jogos","toleranceGames","","Diferença inicial que não entra no cálculo."),
              field("Compensação","compensationPercentage","%","Percentual da oportunidade perdida que será coberto."),
              field("Valor mínimo para sugestão","minimumAmount","M","Use 0 para não definir mínimo. Valores abaixo disso não geram sugestão."),
              field("Valor máximo inicial","maximumAmount","M","Use 0 para não limitar o empréstimo inicial."),
              field("Desconto das recompensas","repaymentPercentage","%","Parte de cada recompensa usada para pagar a dívida.")),
            React.createElement("label",{style:{display:"flex",alignItems:"center",gap:8,fontSize:12,fontWeight:800,margin:"14px 0 10px"}},React.createElement("input",{type:"checkbox",checked:values.allowTopUps,onChange:event=>setValues({...values,allowTopUps:event.target.checked})}),"Permitir complementos"),
            values.allowTopUps&&React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(175px,1fr))",gap:10}},
              field("Valor mínimo para complemento","minimumTopUpAmount","M","Use 0 para não definir mínimo."),
              field("Valor máximo por complemento","maximumTopUpAmount","M","Use 0 para não limitar cada complemento."),
              field("Limite total por usuário","maximumTotalAmount","M","Use 0 para não limitar o total liberado.")),
            React.createElement("button",{onClick:()=>onSave(values),style:{...M,marginTop:14}},"Salvar configurações"),
            React.createElement("div",{style:{marginTop:20,fontSize:13,fontWeight:850}},"Análise dos participantes"),
            React.createElement("div",{style:{display:"grid",gap:10,marginTop:10}},analyses.map(({team,analysis})=>{
              let existing=analysis.loan,suggestion=existing?analysis.topUpSuggested:analysis.suggested,value=amounts[team.id]!=null?amounts[team.id]:suggestion;
              let maxInput=analysis.settings.maximumTotalAmount>0?Math.max(0,analysis.settings.maximumTotalAmount-analysis.totalGranted):undefined;
              return React.createElement("div",{key:team.id,style:{padding:14,borderRadius:15,border:"1px solid var(--border)",background:"var(--surface-soft)"}},
                React.createElement("div",{style:{display:"flex",justifyContent:"space-between",gap:12,alignItems:"flex-start"}},
                  React.createElement("div",null,React.createElement("strong",null,team.name),React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:4}},`${analysis.played} jogos · referência ${analysis.referenceGames}`)),
                  React.createElement("strong",{style:{color:suggestion>0?"var(--green)":"var(--muted)",whiteSpace:"nowrap"}},suggestion>0?`${existing?"+ ":""}${L(suggestion)}`:"Sem sugestão")),
                React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(125px,1fr))",gap:8,marginTop:12,fontSize:11.5}},
                  [["Diferença",analysis.rawGap+" jogos"],["Tolerância",analysis.settings.toleranceGames+" jogos"],["Jogos calculados",analysis.compensableGames],["Média por jogo",L(analysis.averageReward)],["Compensação",analysis.settings.compensationPercentage+"%"],["Valor calculado",L(analysis.calculated)]].map(([label,detail])=>React.createElement("div",{key:label,style:{padding:9,borderRadius:10,background:"var(--surface)",border:"1px solid var(--border)"}},React.createElement("div",{style:{fontSize:10,color:"var(--muted)",marginBottom:3}},label),React.createElement("strong",null,detail)))),
                !analysis.hasEnoughData&&React.createElement("div",{style:{fontSize:11.5,color:"var(--danger)",marginTop:10}},"Ainda não há dados suficientes de recompensas para calcular uma sugestão."),
                existing&&React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:10}},`Já liberado: ${L(existing.totalGranted)} · falta pagar: ${L(existing.remaining)}`),
                existing&&analysis.missedSinceGrant>0&&React.createElement("div",{style:{fontSize:11.5,color:suggestion>0?"var(--green)":"var(--muted)",marginTop:7,lineHeight:1.45}},`Desde a última liberação, a referência avançou ${analysis.referenceIncrease} jogo(s) e ${team.name} jogou ${analysis.playerGamesSinceGrant}. A sugestão foi reavaliada automaticamente${suggestion>0?` e há ${L(suggestion)} de complemento disponível`:""}.`),
                suggestion>0&&React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr auto",gap:8,marginTop:10}},React.createElement("input",{type:"number",min:1,...(maxInput!=null?{max:Math.max(1,maxInput)}:{}),value,onChange:event=>setAmounts({...amounts,[team.id]:Math.max(0,Math.round(Number(event.target.value)||0))}),style:q}),React.createElement("button",{onClick:()=>{let amount=Math.round(Number(value)||0);if(window.confirm(`${existing?"Liberar complemento":"Liberar empréstimo"} de ${L(amount)} para ${team.name}?`))onGrant(team.id,amount)},style:{...M,...W,margin:0,padding:"10px 14px"}},existing?"Atualizar empréstimo":"Dar empréstimo")));
            })));
        }
        ...
        function AdminArea({ currentTournament, tournaments, teams, profiles, profileName, setProfileName, profileColor, setProfileColor, profileBudget, setProfileBudget, tournamentName, setTournamentName, onCreateProfile, onCreateTournament, onDeleteTournament, onSelectTournament, onUpdateBudget, onToggleParticipant, onDeleteProfile, onResetTournament, onRemoveOrphanParticipant, onRestoreOrphanProfile, onUpdateMarketDepreciation, onUpdateInitialRosterDepreciation, onUpdateMarketBalanceRules, onUpdateMarketAccessRules, onUpdateRosterRules, onUpdateEconomyRules, onUpdateBalanceLoanSettings, onGrantBalanceLoan, onFinishTournament, catalog, onImportRosters, onImportHistoricalCompetition }) {
          ...
          return React.createElement("div", null,
            ...
            adminSection === "rules" && currentTournament && React.createElement("section", { style:{ ...E,padding:20,marginTop:18 } },
              ...
              React.createElement(BalanceLoanAdmin,{tournament:currentTournament,teams,onSave:onUpdateBalanceLoanSettings,onGrant:onGrantBalanceLoan}),
