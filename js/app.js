      ...
      `,
        ...
        function ChampionshipRulesModal({ tournament, onClose }) {
          ...
          let ruleTeams=tournament&&tournament.context&&Array.isArray(tournament.context.teams)?tournament.context.teams.filter((team)=>team&&team.active!==false):[];
          let ruleOwnership=tournament&&tournament.context&&tournament.context.ownership&&typeof tournament.context.ownership==="object"?tournament.context.ownership:{};
          let balanceTable=ruleTeams.map((team)=>({team,overall:Number(fullSquadOverall(team.id,ruleOwnership,n)||0)})).filter((entry)=>entry.overall>0).sort((a,b)=>b.overall-a.overall);
