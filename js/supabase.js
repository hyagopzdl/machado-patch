(() => {
  ...
  async function selectPaged(table, columns, configure) {
    const pageSize = 100;
    const rows = [];
  ...
  async function loadNormalizedState() {
    ...
    const [participants, teams, matches, ownership] = await loadGroup([
      () => selectPaged("tournament_participants", "tournament_id,profile_id,position", q => q.order("tournament_id").order("position").order("profile_id")),
      () => selectPaged("teams", "id,tournament_id,profile_id,name,color,budget,active,historical,lineup,source_order", q => q.order("tournament_id").order("source_order").order("id")),
      () => selectPaged("matches", "id,tournament_id,home_team_id,away_team_id,home_profile_id,away_profile_id,stage,round,leg,status,played,home_score,away_score,played_at,created_at,source_order", q => q.order("tournament_id").order("source_order").order("id")),
    ...
    const [stats, offers, histories, transfers] = await loadGroup([
      () => selectPaged("player_stats", "tournament_id,player_id,team_id,player_name_snapshot,goals,red_cards,updated_at", q => q.order("tournament_id").order("player_id")),
      () => selectPaged("trade_offers", "id,tournament_id,player_id,player_name,buyer_team_id,seller_team_id,buyer_profile_id,seller_profile_id,current_amount,market_value_at_creation,last_actor_team_id,status,expires_at,created_at,updated_at", q => q.order("tournament_id").order("id")),
      () => selectPaged("trade_offer_history", "id,offer_id,actor_team_id,action_type,amount,created_at", q => q.order("created_at").order("id")),
      () => selectPaged("transfers", "id,tournament_id,player_id,player_name,transfer_type,from_team_id,to_team_id,offer_id,price,market_value,depreciation_pct,transfer_date,created_at", q => q.order("created_at").order("id"))
    ]);
    ...
    const [favorites, presence, globalOwnership, imports] = await loadGroup([
      () => selectPaged("profile_favorites", "tournament_id,profile_id,player_id,created_at", q => q.order("tournament_id").order("profile_id").order("player_id")),
      () => select("presence", "profile_id,online,updated_at"),
      () => selectPaged("global_player_ownership", "player_id,team_id,for_sale", q => q.order("player_id")),
  ...
  Object.assign(window.ManchaApp,{Ee,U,Q,setTeamBudget,normalizeIdentityText,stableIdentityId,migrateStableIdentitySchema,IDENTITY_SCHEMA_VERSION,supabaseClient:client,fetchSupabasePage:fetchPage});
})();
